...
alter table store add constraint FK_businessman_store2 foreign key (username)
      references user (username) on delete restrict on update restrict;

alter table user add constraint FK_businessman_store foreign key (store_id)
      references store (store_id) on delete restrict on update restrict;

alter table user add constraint FK_user_has_attributes foreign key (use_username)
      references user_attributes (username) on delete restrict on update restrict;

alter table user_attributes add constraint FK_user_has_attributes2 foreign key (username)
      references user (username) on delete restrict on update restrict;
